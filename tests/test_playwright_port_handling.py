...
async def test_port_3001_unavailable():
    ...
    try:
        ...
        assert result != 0, "Failed to block port 3001 for testing"
        ...
        # Attempt to launch the server - this should fail with a clear error
...
async def test_playwright_existing_mcp_server_detection(client):
    ...
    Since we can't reliably have a Playwright MCP server running for the test,
    this test will verify the connection logic or skip if no server is available.
    """
    # Make sure server is in config
    servers = client.list_servers()
    if "playwright" not in servers:
    ...
    # Check if port 3001 is in use by an MCP server
    import socket
    import http.client
    import json
    ...
    if not port_in_use:
        pytest.skip("Port 3001 is not in use, skipping existing server detection test")
